...
def get_args_parser():
    ...
    parser.add_argument('--input_dir',default='roadData/val2017',type=str)
...
def main(args):
    ...
    model = torch.load('mobile_model.pt',map_location='cuda')
    # ------------------------------------把权重加载到gpu或cpu上
    model.to(device)
    ...
    image_file_path = os.listdir(inputDir)
    image_set = []

    for image_item in image_file_path:
        print("inference_image:", image_item)
        image_path = os.path.join(inputDir, image_item)
